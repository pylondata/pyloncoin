...
*Setup instructions for a Gitian build of PYLON Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PYLON
...
- [Building PYLON Core](#building-PYLON-core)
...
Clone the git repositories for PYLON and Gitian.
...
in the PYLON repository under 'Fetch and create inputs' to install sources which require
...
To build PYLON Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the PYLON repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/PYLON/.git/
...
URL=https://github.com/thrasher-/PYLON.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit PYLON=${COMMIT} --url PYLON=${URL} ../PYLON/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit PYLON=${COMMIT} --url PYLON=${URL} ../PYLON/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit PYLON=${COMMIT} --url PYLON=${URL} ../PYLON/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the PYLON git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../PYLON/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/PYLON
SIGPATH=/some/root/path/PYLON-detached-sigs

./bin/gbuild --url PYLON=${BTCPATH},signature=${SIGPATH} ../PYLON/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/PYLON-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/PYLON-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/PYLON-osx-build.assert
```
...
[PYLON-project/gitian.sigs.ltc](https://github.com/PYLON-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
