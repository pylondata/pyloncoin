...
CPubKey CWallet::GenerateNewKey()
{
    ...
    if (!hdChain.masterKeyID.IsNull()) {
        ...
        do
        {
            ...
            std::stringstream ss;
            ss << hdChain.nExternalChainCounter;
            metadata.hdKeypath     = "m/0'/0'/" + ss.str() + "'";
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex)
        {
            if (pindex->nHeight % 100 == 0 && dProgressTip - dProgressStart > 0.0)
                ShowProgress(_("Rescanning..."), std::max(1, std::min(99, (int)((Checkpoints::GuessVerificationProgress(chainParams.Checkpoints(), pindex) - dProgressStart) / (dProgressTip - dProgressStart) * 100))));
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                CAmount nFeeNeeded = ::minRelayTxFee.GetFee(nBytes);
...
{
    {
        ...
        if (fBroadcastTransactions)
        {
            ...
            if (!wtxNew.AcceptToMemoryPool(maxTxFee, state));
            {
                ...
                LogPrintf("CommitTransaction(): Error: Transaction cannot be broadcast: %s\n", state.GetRejectReason());
                return false;
            }
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            LogPrint("keepool", "keypool added key %d, size=%u\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
...
void CWallet::ReserveKeyFromKeyPool(int64_t& nIndex, CKeyPool& keypool)
{
    ...
    keypool.vchPubKey = CPubKey();
    {
        ...
        LogPrint("keepool", "keypool reserve %d\n", nIndex);
    }
}
...
void CWallet::KeepKey(int64_t nIndex)
{
    ...
    LogPrint("keepool", "keypool keep %d\n", nIndex);
}
...
void CWallet::ReturnKey(int64_t nIndex)
{
    ...
    LogPrint("keepool", "keypool return %d\n", nIndex);
}
