...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"pyloncoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Pyloncoin address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"pyloncoinaddress\"   (string, required) The Pyloncoin address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Pyloncoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (!masterKeyID.IsNull())
    {
        ...
        if (pwalletMain->GetKey(masterKeyID, key))
        {
            ...
            file << "# extended private masterkey: " << b58extkey.ToString() << " (" << HexStr(key.begin(), key.end()) << ")" << "\n\n";
        }
    }
