...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Pyloncoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Pyloncoin Core is probably already running."), addrBind.ToString());
        else
...
{
    LOCK(cs_totalBytesSent);
    uint64_t recommendedMinimum = (nMaxOutboundTimeframe / 600) * dynParams.nMaxBlockSize;
...
bool CNode::OutboundTargetReached(bool historicalBlockServingLimit)
{
    ...
    if (historicalBlockServingLimit)
    {
        ...
        uint64_t buffer = timeLeftInCycle / 600 * dynParams.nMaxBlockSize;
