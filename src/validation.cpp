...
#if defined(NDEBUG)
# error "PYLON cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "PYLON Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting)
                    {
                        LogPrint("mempool", "replacing tx %s with %s for %s PYLNC additional fees, %d delta bytes\n",
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // PYLON: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
